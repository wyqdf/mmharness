"""Cross-retriever majority vote: 3 parallel retrievals with voting.

Runs THREE distinct retrievers (word-BM25, char-ngram BM25, label-centroid)
in parallel and votes on their LLM answers. Different retrievers surface
different examples, providing complementary evidence. Tie-breaks to centroid.
"""
...
class CrossRetrieverVote(MemorySystem):
    """Cross-retriever vote with word/char/centroid BM25."""
    ...
    def _ensure_index(self):
        ...
        # Build label centroids (combined word+char)
    ...
    def predict(self, input: str) -> tuple[str, dict[str, Any]]:
        ...
        # Retrieve with 3 different strategies
        word_idxs = self._retrieve_word(input, TOP_K)
        char_idxs = self._retrieve_char(input, TOP_K)
        centroid_idxs = self._retrieve_centroid(input, TOP_K)
        ...
        # Vote (tie-break to centroid = answers[0])
        counts = Counter(answers)
        most_common = counts.most_common()
        max_count = most_common[0][1]
        winners = [ans for ans, cnt in most_common if cnt == max_count]
        final_answer = winners[0] if len(winners) == 1 else answers[0]

        return final_answer, {"answers": answers, "vote": final_answer}
