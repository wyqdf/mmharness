"""Centroid BM25 with completion recheck: two-stage predict for multi-label tasks.

Stage 1: standard label-centroid BM25 retrieval + LLM predict.
Stage 2 (gated): if retrieved labels contain set separators (;,、), run
verification LLM call asking "did you miss any items?" and merge new items.

Targets LawBench (58% multi-label) while maintaining USPTO/S2D (gate closed).
"""
...
VERIFY_PROMPT = """You previously gave the following tentative answer:
{tentative}

Now check whether the answer is COMPLETE. The input may contain multiple items
(e.g., multiple charges, multiple diseases). Examples of multi-item answers:
{multi_examples}

**Problem:**
{input}

Did you miss any items? If yes, provide ONLY the missing items. If no, respond "complete".
Respond in JSON: {{"missing": "[missing items or 'complete']"}}"""
...
class CentroidCompletionRecheck(MemorySystem):
    """Centroid BM25 with gated completion verification."""
    ...
    def _detect_separator(self, labels: list[str]) -> str | None:
        """Detect if labels contain set-style separators."""
        seps = [';', ',', '、', '；']
    ...
    def predict(self, input: str) -> tuple[str, dict[str, Any]]:
        ...
        # Gate: check if retrieved labels suggest multi-label task
        retrieved_labels = [self.examples[i]["target"] for i in idxs[:20]] if idxs else []
        sep = self._detect_separator(retrieved_labels)

        if sep:
            # Stage 2: verification call
            multi_ex = [ex for ex in retrieved_labels if sep in ex][:5]
            multi_section = "\n".join(multi_ex) if multi_ex else "N/A"
            verify_prompt = VERIFY_PROMPT.format(
                tentative=tentative, multi_examples=multi_section, input=input
            )
            verify_resp = self.call_llm(verify_prompt)
            missing = extract_json_field(verify_resp, "missing")

            if missing and missing.lower() not in ["complete", "none", "no", "n/a"]:
                # Merge new items
                tent_parts = {p.strip() for p in tentative.split(sep) if p.strip()}
                new_parts = {p.strip() for p in missing.split(sep) if p.strip()}
                merged = tent_parts | new_parts
                answer = sep.join(sorted(merged))
            else:
                answer = tentative
        else:
            answer = tentative

        return answer, {"full_response": response, "num_examples": len(self.examples), "gate_fired": sep is not None}
