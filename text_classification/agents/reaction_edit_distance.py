"""Reaction-edit-distance retrieval: stratified by type, ranked by Levenshtein.

Builds on reaction_stratified_retrieval (USPTO 0.267) by replacing char-ngram
Jaccard with SMILES edit distance. Edit distance captures structural similarity
better: CC(C)C -> CC(C)CC (dist=1) vs CC(C)C -> c1ccccc1 (dist=8). Lower
distance = higher similarity for reaction transformations.
"""
...
class ReactionEditDistance(MemorySystem):
    """Stratified retrieval using reaction types and SMILES edit distance."""
    ...
    def _retrieve_examples(self, input: str, max_k: int = 40) -> list[dict]:
        ...
        if reaction_type and reaction_type in self.reaction_groups:
            ...
            for idx in candidate_indices:
                ...
                dist = _levenshtein(query_smiles, ex_smiles)
                sim = 1.0 / (1.0 + dist)
